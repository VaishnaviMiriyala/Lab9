...
public class StreamDemo {
	...
	void intermediate_FilterOperation()
	{
		...
		//Employee details by max salary
		Employee res = stream.filter((employee)->employee.getProjectName().equals("p1")).max(bySalaryComparator).get();
	...
	void concatMethod() {
		Stream<String> stream1 = Stream.of("Raj", "Shyam");
        Stream<String> stream2 = Stream.of("xyz", "abc");
        Stream<String> stream3 = Stream.of("xyzdjh", "shGabc");
        //Stream.concat(stream1, stream2).forEach(element -> System.out.println(element));
        
       // Stream.concat(Stream.concat(stream1, stream2),stream3).forEach(element -> System.out.println(element));
        
        Stream.concat(stream1, Stream.concat(stream2,stream3)).forEach(element -> System.out.println(element));
	}
